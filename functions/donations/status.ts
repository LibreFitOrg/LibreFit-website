...
/** Known donation statuses reported by the Trocador webhook. */
type TrocadorStatus =
  | 'anonpaynew'
  | 'waiting'
  | 'confirming'
...
} satisfies Record<TrocadorStatus, StatusInfo>;

function getStatus(status: string | null): StatusInfo {
  return STATUSES[status as TrocadorStatus] || { title: '❓ Unknown Status', description: 'An unknown status was received. Please contact support.' };
}
...
export const onRequestGet: PagesFunction<Env> = async ({ request, env }) => {
  ...
  if (records.length != 0){
    ...
    code = donation.code ?? code;

    const waitingKeywords: (string | null)[] = ["anonpaynew", "waiting", "confirming", "sending", "paid_partially"];
